...
- **Chưa bù sau giải trình**: trạng thái một buổi gốc đã có **giải trình vắng** nhưng chưa có **lịch bù** tương ứng (`baselineScheduleEntryId` + `originalDate`). Trên UI, dòng trong card **Cảnh báo chưa dạy** chuyển sang **Đã giải trình · chưa bù**, hiển thị lý do đã nhập và CTA **Xếp lịch bù**; dòng biến mất khi đã xếp **lịch bù**. Trong phạm vi lớp còn `running` và gia sư còn active trên lớp, mọi ca giải trình đều phải được xếp bù — không có lối thoát **không cần bù**.

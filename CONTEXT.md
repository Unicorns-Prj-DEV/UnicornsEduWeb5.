...
- **Chưa bù sau giải trình**: trạng thái một buổi gốc đã có **giải trình vắng** nhưng chưa có **lịch bù** tương ứng (`baselineScheduleEntryId` + `originalDate`). Trên UI, dòng thu gọn trong card **Cảnh báo chưa dạy** hiện badge **Đã giải trình · chưa bù**; mở accordion mới thấy lý do đã nhập và form **Xếp lịch bù**; dòng biến mất khi đã xếp **lịch bù**. Trong phạm vi lớp còn `running` và gia sư còn active trên lớp, mọi ca giải trình đều phải được xếp bù — không có lối thoát **không cần bù**.

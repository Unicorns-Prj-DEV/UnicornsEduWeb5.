...
- **CD Prisma behavior**: GitHub Actions deploy runs `prisma migrate deploy` on each enabled VPS instance via `scripts/prisma-migrate-deploy.sh` (mounted into the API container). The script uses `DIRECT_URL`, or derives Supavisor session mode (`pooler.supabase.com:5432`) from `DATABASE_URL` when `DIRECT_URL` is missing or points at IPv6-only `db.*.supabase.co`. **Runtime API** still connects via PgBouncer using `DATABASE_URL` in `PrismaService`. Do not run `prisma migrate dev` in CD.

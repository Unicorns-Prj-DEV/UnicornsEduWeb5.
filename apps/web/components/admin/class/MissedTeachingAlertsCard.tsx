...
type MissedTeachingAlertsCardProps = {
  ...
  onMakeupCreated?: () => Promise<void> | void;
...
export default function MissedTeachingAlertsCard({
  ...
  const handleCreateMakeup = async (alert: MissedTeachingAlert) => {
    ...
    if (!reason) {
      toast.error("Vui lòng nhập lý do giải trình trước khi xếp lịch bù.");
      return;
    }
              ...
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.9fr)]">
                ...
                {canCreateMakeup ? (
                  <div className="grid gap-2 sm:grid-cols-[1fr_0.8fr_0.8fr]">
                    ...
                    <textarea
                      value={draft.reason}
                      onChange={(event) => updateDraft(alert, { reason: event.target.value })}
                      placeholder="Lý do giải trình"
                      className="min-h-20 rounded-md border border-border-default bg-bg-primary px-3 py-2 text-sm text-text-primary outline-none transition focus:border-border-focus focus:ring-2 focus:ring-border-focus/25 sm:col-span-3"
                      required
                    />
                    <button
                      type="button"
                    ...
                    >
                      {submittingId === alert.id ? "Đang xếp..." : "Xếp lịch bù"}
                    </button>
                  </div>
                ) : null}
              </div>

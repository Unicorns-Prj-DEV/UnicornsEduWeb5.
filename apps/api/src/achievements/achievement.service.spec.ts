...
describe('AchievementService.assertCompleteReorder', () => {
  const service = Object.create(AchievementService.prototype) as AchievementService;
  ...
  it('rejects duplicates and foreign ids', () => {
    expect(() =>
      service.assertCompleteReorder(['a', 'b'], ['a', 'a']),
    ).toThrow(BadRequestException);
    expect(() =>
      service.assertCompleteReorder(['a', 'b'], ['a', 'z']),
    ).toThrow(BadRequestException);
  });
});

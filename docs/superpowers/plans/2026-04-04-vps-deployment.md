...
server {
    ...
    # With variable proxy_pass, adding a URI part like `...:4000/` would
    # collapse every `/api/*` request to `/` on the upstream. Rewrite first,
    # then proxy without a URI so the remainder of the path is preserved.
    location /api/ {
        set $upstream_api api;
        rewrite ^/api/?(.*)$ /$1 break;
        proxy_pass         http://$upstream_api:4000;

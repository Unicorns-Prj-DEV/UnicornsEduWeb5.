...
- Đường dẫn cây nội dung (chủ đề / chuyên đề / bài học): href trang Next chỉ qua `apps/web/lib/course-content-routes.ts`; URL Axios chỉ qua `apps/web/lib/content-api-paths.ts`. Không tự ghép chuỗi chứa `/chapters`, `/topics`, `/lectures` ở component hay `lib/apis/*`.
...
- Fetch `modules` / `difficulty-levels` của khoá dùng hook chung `useCourseModules` / `useCourseDifficultyLevels` (`apps/web/lib/hooks/`). Ô search server-side (BankPicker, `PracticeLessonQuestionsCard`, tab Đề thi workspace khoá) debounce ~300ms (`use-debounce`).

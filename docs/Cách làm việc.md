...
- Đường dẫn cây nội dung (chuyên đề / tiết học): href trang Next chỉ qua `apps/web/lib/course-content-routes.ts`; URL Axios chỉ qua `apps/web/lib/content-api-paths.ts`. Không tự ghép chuỗi chứa `/chapters`, `/topics`, `/lectures` ở component hay `lib/apis/*` — các segment đó đã khai tử.
- Mutation trong phạm vi khoá/lớp invalidate key theo `courseId`/`classId` (`questionKeys.course`, `examLibraryKeys.course`, `courseKeys.modules`, `courseKeys.lessonsPrefix`, `courseKeys.difficultyLevelsPrefix`) qua helper `apps/web/lib/query-invalidation.ts` — không invalidate `*.all` khi đã biết phạm vi. Tiết thực hành cấp khoá vẫn có thể đọc qua `examLibraryKeys` (lần giao lớp) và `courseKeys.lessons`; mutation gọi `invalidateCoursePracticeLessonQueries(queryClient, courseId)`. Không dùng `staleTime: 0` để lách.
- Fetch `modules` / `difficulty-levels` của khoá dùng hook chung `useCourseModules` / `useCourseDifficultyLevels` (`apps/web/lib/hooks/`). Ô search server-side (BankPicker, `PracticeLessonQuestionsCard`) debounce ~300ms (`use-debounce`).

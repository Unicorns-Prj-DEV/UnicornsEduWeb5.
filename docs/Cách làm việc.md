...
CD trên VPS chạy `scripts/gha-deploy-remote.sh`: pull image GHCR mới, apply Prisma migration từ chính API image bằng `prisma migrate deploy`, rồi recreate `api`/`web`/`nginx` và healthcheck. Khi đổi Prisma schema cho production/shared DB, phải commit migration trong `apps/api/prisma/schema/migrations/`; không dùng `prisma migrate dev` trên database shared.

...
Email transactional của auth (`register`, `resend-verification`, `forgot-password`) gửi qua Resend trong `src/mail/mail.service.ts`. Cấu hình backend bắt buộc: `RESEND_API_KEY` và `MAIL_FROM`; không dùng SMTP env (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_SECURE`) nữa. Theo best practices của Resend, API key chỉ nằm trong private backend env, `MAIL_FROM` dùng sender/domain đã verify trên Resend ở production, lỗi `429`/`500` mới retry có backoff và các request retry phải đi kèm idempotency key.

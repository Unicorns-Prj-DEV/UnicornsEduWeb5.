...
- **Luồng session:** resolve **vận hành** trực tiếp từ `class_teachers.operatingDeductionRatePercent` — không còn bảng lịch sử/fallback song song; **không** gán cột này vào `teacher_tax_deduction_rate_percent` (thuế thu nhập).
...
- Kiểm tra **session create/update/payment** snapshot operating đúng từ source duy nhất `class_teachers.tax_rate_percent`, gồm case `0%`.

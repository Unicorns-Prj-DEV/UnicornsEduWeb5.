...
- `/user-profile` đã đồng bộ section `Nhân sự` với form chi tiết nhân sự: hỗ trợ đủ field hồ sơ staff (bao gồm `cccd_number`, `cccd_issued_date`, `cccd_issued_place`, `personal_achievement_link` / minh chứng thành tích) và upload ảnh CCCD 2 mặt qua `POST /users/me/staff/cccd-images`

...
| `apps/api`     | NestJS backend. `src/` (auth, `action-history/`, `cache/`, `notification/`, prisma, `session/` workflow services, `staff-ops/` access helpers, user/student/staff services, mail qua Resend, …), `prisma/schema/`, `generated/` (Prisma Client), `dtos/`. Runtime hiện dùng bảng `dashboard_cache` của PostgreSQL cho dashboard read cache, auth identity cache in-memory TTL ngắn cho guard/profile lookups, global HTTP rate limiting qua `@nestjs/throttler`, và gateway `/notifications` để push realtime notification theo audience cho admin/staff/student. |
...
- Guard route staff: khi `roleType=staff` nhưng hồ sơ chưa đủ các trường bắt buộc (nhóm user: tên/email/phone/province; nhóm staff: CCCD + ngày/nơi cấp + ngày sinh + học vấn/chuyên môn + ngân hàng + ảnh CCCD 2 mặt), proxy sẽ redirect về `/user-profile?profile_required=1&from=...` để hoàn thiện thông tin trước khi vào `/staff/**`
...
- `/user-profile` đã đồng bộ section `Nhân sự` với form chi tiết nhân sự: hỗ trợ đủ field hồ sơ staff (bao gồm `cccd_number`, `cccd_issued_date`, `cccd_issued_place`) và upload ảnh CCCD 2 mặt qua `POST /users/me/staff/cccd-images`

...
- Guard route staff: khi `roleType=staff` nhưng hồ sơ chưa đủ các trường bắt buộc (nhóm user: tên/email/phone/province; nhóm staff: CCCD + ngày/nơi cấp + ngày sinh + học vấn/chuyên môn + ngân hàng + ảnh CCCD 2 mặt), proxy redirect về `/user-profile?profile_required=1&from=...` để hoàn thiện thông tin trước khi vào `/staff/**`
- Khi vào `/user-profile` với cờ `profile_required=1`, frontend hiển thị cảnh báo bắt buộc hoàn thiện hồ sơ (banner + warning toast)
- `/user-profile` đồng bộ section `Nhân sự` với form chi tiết: `cccd_*`, học vấn, ngân hàng, `personal_achievement_link`, upload ảnh CCCD qua `POST /users/me/staff/cccd-images`; đồng thời vẫn có thể chỉnh hồ sơ qua `/staff/profile` (popup `StaffSelfEditPopup`)
...
- **AdminAccessGate** mở rộng: `assistant` có full access `/admin/**`; `accountant` truy cập `/admin/dashboard`, `/admin/classes`, `/admin/classes/[id]`, `/admin/staffs`, `/admin/staffs/[id]`, `/admin/costs`, `/admin/lesson-plans`, `/admin/accountant_detail`, `/admin/assistant_detail`, `/admin/communication_detail`, `/admin/technical_detail`, `/admin/customer_care_detail/[staffId]`, `/admin/lesson_plan_detail/[staffId]`; `lesson_plan_head` truy cập `/admin/lesson-plans/**`.

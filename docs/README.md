...
- Route `/student` hiện là self-service page của chính học sinh đang đăng nhập, bám layout của `/admin/students/[id]`, cho phép tự cập nhật thông tin cơ bản; học phí/gói học phí theo lớp được mở ở chế độ chỉ đọc, còn các control tài chính nhạy cảm vẫn bị ẩn.
  - `/student` mở khi tài khoản có linked `studentInfo` hợp lệ và lấy dữ liệu qua `/users/me/student-detail`, `/users/me/student`, `/users/me/student-wallet-history`, `/users/me/student-wallet-sepay-topup-order`, `/users/me/student-exam-schedules`
  ...
  - popup ví trên `/student` chỉ cho học sinh tự tạo QR SePay để nạp tiền; backend khóa theo đúng hồ sơ hiện tại và không cho self-service chỉnh thẳng số dư hoặc rút tiền
- Route `/staff` hiện có staff shell với sidebar theo role:
  ...
  - Sidebar của assistant chuyển sang menu admin-like trong staff shell: **Dashboard**, **User**, **Nhân sự**, **Lớp học**, **Ghi chú môn học**, **Học sinh**, **Chi phí**, **Giáo Án**, **Lịch sử**; sidebar kế toán có thêm **Nhân sự** và **Học sinh** bên cạnh các mục tài chính/giáo án
...
- Guard route staff: khi `roleType=staff` nhưng hồ sơ chưa đủ các trường bắt buộc (nhóm user: tên/email/phone/province; nhóm staff: CCCD + ngày/nơi cấp + ngày sinh + học vấn/chuyên môn + ngân hàng + ảnh CCCD 2 mặt), proxy sẽ redirect về `/user-profile?profile_required=1&from=...` để hoàn thiện thông tin trước khi vào `/staff/**`. `personal_achievement_link` là minh chứng thành tích tùy chọn, không nằm trong guard bắt buộc.
...
- `/user-profile` đã đồng bộ section `Nhân sự` với form chi tiết nhân sự: bộ đếm section này chỉ tính 12 field hồ sơ staff do người dùng tự hoàn thiện, không tính metadata hệ thống `status`/`roles`; hỗ trợ đủ field hồ sơ staff (bao gồm `cccd_number`, `cccd_issued_date`, `cccd_issued_place`, `personal_achievement_link`) và upload ảnh CCCD 2 mặt qua `POST /users/me/staff/cccd-images`; đồng thời vẫn có thể chỉnh hồ sơ qua `/staff/profile` (popup `StaffSelfEditPopup`)
...
- **Staff/student shell policy dùng chung**: `StaffAccessGate` và `proxy.ts` cùng dùng `staff-shell-access` cho `/staff/**`; `/student/**` cũng được proxy chặn sớm bằng session nhẹ (`roleType=student` + `hasStudentProfile`) trước khi client gate render.

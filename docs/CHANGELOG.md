...
- CD deploy IT: tự `docker compose -p unicorns down` khi migrate sang project `unicorns-it` (tránh port 80 already allocated). bằng cách chuyển đổi việc lưu lịch sử hoạt động (Action History) từ vòng lặp $O(N)$ query ghi log tuần tự sang chèn hàng loạt (Bulk Insert) $O(1)$ query thông qua phương thức `recordUpdates` mới trong `ActionHistoryService`, giúp loại bỏ hoàn toàn hiện tượng nghẽn cơ sở dữ liệu khi thanh toán số lượng lớn mục.

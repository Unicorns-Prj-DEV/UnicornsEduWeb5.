...
- BE `apps/api`: Tối ưu hóa hiệu năng tính năng "Thanh toán tất cả" (Pay All) trong `StaffService.applyStaffPaymentPreviewRecords` bằng cách chuyển đổi việc lưu lịch sử hoạt động (Action History) từ vòng lặp $O(N)$ query ghi log tuần tự sang chèn hàng loạt (Bulk Insert) $O(1)$ query thông qua phương thức `recordUpdates` mới trong `ActionHistoryService`, giúp loại bỏ hoàn toàn hiện tượng nghẽn cơ sở dữ liệu khi thanh toán số lượng lớn mục.

...
- **Staff endpoints & frontend data fetching:**
  ...
  - Session allowance trong `income-summary` được tính ở DB layer theo công thức `min(max_allowance_per_session, (allowance_amount * attended_students + scale_amount) * coefficient)`, trong đó `attended_students` đếm cả attendance `present` và `excused`. Giá trị `max_allowance_per_session` null hoặc `0` được coi như không có trần (cùng semantics với SQL `NULLIF(..., 0)` trước khi áp `min`).

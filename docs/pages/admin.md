...
- **Calendar tuần:** `/admin/calendar` dùng `Class.schedule` làm source of truth và chỉ hiển thị lịch học của **tuần hiện tại** theo layout week-view kiểu Google Calendar, cố định từ **CN -> T7**; FE giữ filter lớp dưới dạng **combobox tìm theo tên** và thêm filter single-select theo **gia sư chịu trách nhiệm** của từng slot lịch, không dùng pagination hay chọn khoảng ngày thủ công. Event card và popup không còn hiển thị gia sư của từng buổi; nếu lớp đã sync recurring event lên Google Calendar thì popup đọc trực tiếp `meetLink` từ schedule occurrence để mở link lớp. Trên mobile, week-view giữ đủ 7 ngày bằng vùng cuộn ngang có chủ đích, filter xếp dọc mobile-first, và popup sự kiện chuyển sang kiểu bottom-sheet cuộn nội bộ để không tràn viewport.

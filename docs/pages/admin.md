...
- **Save/refetch UX cho CRUD class/staff/student:** mọi action **Save** không mang tính destructive (tạo/sửa lớp, gia sư, học sinh, session, học phí, ví, gán lớp, hồ sơ tự sửa tương ứng ở route mirror) dùng fast-close UX: pass client validation là đóng popup/thoát edit mode ngay, hiện `toast.loading`, backend mutation chạy nền rồi resolve cùng toast sang success/error; lỗi chỉ hiện toast, không tự mở lại form. Khi list/detail đang refetch mà đã có dữ liệu cũ, UI giữ nguyên dữ liệu cũ, dim nhẹ section và hiện refresh strip/skeleton mảnh thay vì thay cả trang bằng loading skeleton.
...
- **Dashboard aggregate endpoint (admin-only):**
  - `GET /dashboard?month=<01-12>&year=<YYYY>&alertLimit=<1-20>&topClassLimit=<1-20>` trả toàn bộ payload cho `/admin/dashboard`: `period`, `summary`, `revenueProfitTrend`, `breakdown`, `actionAlerts`, `classPerformance`, `yearlySummary`.
    - **Phạm vi thời gian:** với mỗi cặp `month` + `year`, toàn bộ chỉ số tiền (doanh thu đã học, chi phí, lợi nhuận, nạp, prepaid, nợ, payroll pending, breakdown, `revenueProfitTrend`, `yearlySummary`) và các nhóm cảnh báo gắn giao dịch đều **lọc theo đúng tháng dương lịch đó** (`sessions.date`, `lesson_outputs.date`, `wallet_transactions_history.created_at`, `bonuses.month` / `extra_allowances.month` / `cost_extend`, v.v.). Riêng `summary.activeClasses` và `summary.activeStudents` vẫn là snapshot hiện tại (lớp `running`, học sinh `active`).
  ...
  - `GET /dashboard/student-balance-details?limit=<1-500>&month=<01-12>&year=<YYYY>` trả danh sách học sinh active thuộc lớp running có số dư dương (`account_balance > 0`) **và có ít nhất một session trong tháng đang xem**, gồm `studentName`, `className`, `balance`; FE dùng cho popup chi tiết khi bấm vào số tiền **Học phí chưa dạy** (khớp `prepaidTuitionTotal` trên aggregate).
    - Endpoint này dùng `dashboard_cache` theo `limit/month/year`; nếu row cache không tồn tại, hết hạn hoặc thao tác cache lỗi thì backend bỏ qua cache và query DB như cũ.
  ...
  - `summary.monthlyExpense` và breakdown chi phí được cộng từ các nguồn trong **cùng tháng đang xem**: phụ cấp buổi dạy (session allowance), commission CSKH (`attendance.customer_care_coef`), `lesson_outputs.cost`, `bonuses.month`, `extra_allowances.month`, `cost_extend` (theo tháng hoặc date trong tháng).
...
- **Staff endpoints & frontend data fetching:**
  ...
  - `PATCH /staff/:id/class-teachers/:classId/operating-deduction` — body `{ operating_deduction_rate_percent }` (0–100, tối đa 2 chữ số thập phân); cập nhật đúng một dòng `class_teachers`. **Chỉ `UserRole.admin`** được gọi (403 nếu không phải admin). Response trả về cùng shape `GET /staff/:id` sau khi cập nhật.
  ...
  - `GET /staff/:id/payment-preview?month=<01-12>&year=<YYYY>` trả payload popup **Thanh toán**, group theo `role -> source -> items`; mỗi item có `grossAmount`, `operatingAmount`, `taxRatePercent`, `taxAmount`, `netAmount`, và response có thêm `taxAsOfDate`. **Buổi dạy (GV):** mọi session `teacher_payment_status = unpaid` của nhân sự, **không giới hạn** theo `month/year` của query. **Các nguồn khác** (thưởng, trợ cấp, lesson output, hoa hồng CSKH, chia trợ lí…) vẫn chỉ lấy khoản pending trong tháng/năm được truyền. Thuế ở popup luôn được tính lại theo **mức hiện hành của chính staff đang thanh toán** cho từng role; riêng **Giáo viên** dùng base thuế = `gross - operating`. Bonus luôn nằm ở section riêng `Thưởng`, còn session `deposit` bị loại khỏi preview.
...
- FE `/admin/staff/:id` thêm card riêng **Thống kê thuế theo role** (đặt ngay dưới card **Công việc khác**), hiển thị mức thuế hiện hành theo từng role của nhân sự ở tháng đang xem, ưu tiên staff override rồi fallback role default từ `GET /deduction-settings/tax`; card dùng một nút **Chỉnh sửa** ở góc phải, bật chế độ bulk edit để sửa nhiều role cùng lúc ngay tại cột `Thuế hiện hành`, và nếu role chưa có override thì thao tác lưu sẽ tạo override mới cho staff.
  - FE `/admin/staff/:id` thêm nút **Thanh toán** ở góc trên bên phải header. Popup thanh toán lấy dữ liệu từ `GET /staff/:id/payment-preview?month=&year=` (tháng đang xem quyết định các nguồn không phải buổi dạy); **buổi dạy** trong popup là **mọi buổi chưa thanh toán** của GV; group khoản theo role và source, **Thưởng** là section riêng.
  ...
  - Khi xác nhận, FE gọi `PATCH /staff/:id/payment-status/pay-all` với body `{ month, year }`; backend tự tính lại cùng tập như preview (mọi buổi GV chưa thanh toán + pending tháng đó đối với các nguồn khác), refresh snapshot thuế hiện hành theo role, rồi chuyển sang `paid`. FE không gửi danh sách id từ client và sẽ invalidate lại `staff detail`, `income-summary`, `sessions`, `bonus list`, `lesson output stats` sau khi thành công.
  ...
  - FE `/admin/staffs/:id` khi mở trong shell **`/admin`** (route `/admin/staffs/...`): card **Lớp phụ trách** thêm cột **KH vận hành** (%) lấy từ `GET /staff/:id` → `classTeachers[].operatingDeductionRatePercent`; **admin** chỉnh trực tiếp trong bảng/card → `PATCH /staff/:id/class-teachers/:classId/operating-deduction`. Trên mirror **`/staff/staffs/:id`** không hiển thị cột này. Người xem không phải admin trên `/admin/staffs/:id` vẫn thấy % (read-only), không có ô nhập.

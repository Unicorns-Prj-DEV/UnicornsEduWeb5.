...
- **Nạp tiền qua SePay:** Học sinh mở popup nạp ví → frontend gọi `GET /users/me/student-wallet-sepay-static-qr` → hiển thị **QR tĩnh riêng học sinh** không chứa số tiền. QR dùng VietQR/bank-transfer với nội dung `NAPVI UNIST-{uuid} UNICL-{uuid}... LOP <tên lớp...>`; phụ huynh chuyển khoản số tiền cần nạp, webhook SePay parse student id + class ids ở đầu nội dung để cộng ví theo `transferAmount` thực nhận, còn hậu tố `LOP` giúp người nhận đọc được các lớp đang học. API ghi ledger completed trong `student_wallet_sepay_orders` bằng `sepay_transaction_id` / `sepay_reference_code` để chống cộng trùng, tạo `wallet_transactions_history`, cập nhật số dư và gửi **email biên lai nạp ví** tới `parent_email` nếu có, đồng thời gửi riêng cho CSKH đang phụ trách nếu có email. Biên lai có dòng nội dung tiếng Việt để phụ huynh/CSKH đọc được học sinh, lớp gia hạn và số tiền. Endpoint tạo order động cũ vẫn tồn tại để tương thích, nhưng UI chính không còn nhập số tiền hoặc tạo order khi mở QR.
...
- **API (real):**
  ...
  - `GET /users/me/student-wallet-sepay-static-qr` (SePay QR tĩnh, nội dung `NAPVI UNIST-{uuid} UNICL-{uuid}... LOP <tên lớp...>`, không chứa số tiền; response trả thêm `classIds`)

...
- **API (real):**
  ...
  - `POST /webhook/sepay` — SePay gọi khi có giao dịch ngân hàng; API xác thực `SEPAY_WEBHOOK_SECRET`, reconcile theo mã đơn/nội dung CK, trả `{ "status": "success" }` khi nhận hợp lệ.

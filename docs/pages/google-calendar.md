...
- Khi ghi lại `Class.schedule` sau resync, backend **giữ nguyên** `createdAt`/`deletedAt` và các slot soft-deleted (lịch sử đổi lịch). Chỉ cập nhật metadata Google (`googleCalendarEventId`, `meetLink`) trên slot đang active. Nếu slot active thiếu `createdAt` (data cũ bị strip), backfill `createdAt = Class.updatedAt` để **Cảnh báo chưa dạy** không quét giờ mới trên ngày quá khứ.

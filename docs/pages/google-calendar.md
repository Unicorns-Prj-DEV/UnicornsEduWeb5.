...
- **lịch học định kỳ của lớp** (bảng `class_schedule_entries`, xem mục 4.4.0a `Database Schema.md`; `Class.schedule` JSON cũ chỉ còn là backup lịch sử, không được đọc/ghi nữa)
- **lịch dạy bù một lần** (`makeup_schedule_events`)
- **lịch thi của học sinh** (`student_exam_schedules`)

- Mỗi entry trong `class_schedule_entries` có thể được sync thành một recurring event trên Google Calendar.
- Event recurring này có thể kèm Google Meet link, ghi ngược `googleCalendarEventId`/`meetLink` lên đúng entry đó (giữ nguyên `id`, không re-key).
...
- Các màn `/admin/calendar` và `/staff/calendar` hiện render từ **aggregate calendar feed** ở frontend. Feed này gộp:
  - `fixed`: slot cố định expand từ `class_schedule_entries` (slot active, `effectiveTo IS NULL`)
...
`GoogleCalendarModule` được import bởi `CalendarModule` và `StudentModule` để phục vụ sync recurring event của `class_schedule_entries`, event one-off của `makeup_schedule_events`, và all-day exam event của `student_exam_schedules`.
...
| `GET`    | `/admin/calendar/class-schedule`                | Contract cũ chỉ chứa fixed occurrence từ `class_schedule_entries`; FE dùng làm fallback tương thích                                                |
...
| `POST`   | `/class/:classId/schedule/google-calendar/resync` | Admin đầy đủ resync toàn bộ recurring Google Calendar event của lớp từ `class_schedule_entries` (slot active) hiện tại                             |
...
| `POST`   | `/staff-ops/classes/:classId/schedule/google-calendar/resync` | Admin trong staff shell resync toàn lớp; staff thường chỉ resync các slot `class_schedule_entries` có `teacherId` là chính mình                    |
...
- Regenerate thủ công: gọi `POST /staff/:id/regenerate-meet-link` (mọi staff role đều được phép). Backend tạo link mới, cập nhật `staff_info.google_meet_link`, rồi backfill link vào các entry `class_schedule_entries` (slot active) và `makeup_schedule_events` mà gia sư đó phụ trách.
...
5. `entry.meetLink` trên `class_schedule_entries` cũng được set về link lấy từ `staff_info` (backward-compat FE đọc), kể cả khi Google Calendar trả về một link per-event khác.
...
### 4.2 Recurring event cho `class_schedule_entries`

Mỗi schedule entry active trong `class_schedule_entries` (`effectiveTo IS NULL`) có thể được đồng bộ thành một recurring weekly event:
...
- Recurring event mới luôn gắn `extendedProperties.private` gồm `unicornsType=classSchedule`, `unicornsClassId`, `unicornsScheduleEntryId` để backend tìm lại và xoá/reconcile kể cả khi entry legacy thiếu `googleCalendarEventId`.

Khi gọi `PUT /admin/calendar/classes/:classId/schedule` (hoặc `PATCH /class/:id/schedule`), hệ thống sẽ:

1. Ghi schedule pattern mới xuống `class_schedule_entries` theo cơ chế soft-close (`UPDATE effectiveTo` trên entry cũ, `INSERT` entry mới) và **giữ nguyên `id`** khi slot không đổi, giữ lại `googleCalendarEventId`/`meetLink` theo `id` đó nếu slot vẫn tồn tại.
...
3. Với từng entry active hiện tại, ưu tiên `events.update` trên event id đang lưu; nếu id cũ đã bị xoá bên Google (`404`) thì backend tạo event thay thế hoặc update event cùng `unicornsScheduleEntryId` đã discover được.
4. Resolve Meet link từ `staff_info` của gia sư phụ trách (auto-create nếu thiếu).
5. Sau khi các target entry đã sync xong, backend mới xoá orphan/duplicate Google event không còn thuộc slot active hiện tại; delete dùng `sendUpdates=none` và 404 được xem là idempotent success.
6. Lưu `googleCalendarEventId` và `meetLink` (lấy từ `staff_info`) ngược lại vào đúng row `class_schedule_entries` theo `id` (entry đã soft-close/`effectiveTo` không bị đụng, giữ nguyên làm lịch sử).
...
Khi gọi `PATCH /class/:classId/teachers` và một gia sư bị gỡ khỏi lớp, `ClassService.updateClassTeachers()` soft-close toàn bộ entry active trong `class_schedule_entries` có `teacherId` là gia sư bị gỡ (`effectiveTo = hôm nay`, không xoá bản ghi). Sau đó backend gọi `CalendarService.syncScheduleWithCalendar(classId, oldSchedule)` để Google Calendar xoá recurring event của các slot cố định đã bị loại bỏ. Đây là guard backend bắt buộc; FE cũng xoá các slot đó ngay trong popup sửa lớp để người dùng thấy trước thay đổi.

Manual resync dùng cùng nguyên tắc **system schedule là source of truth, Google Calendar là projection**:

- Admin đầy đủ gọi `/class/:classId/schedule/google-calendar/resync` để update/recreate recurring Google event từ các entry active trong `class_schedule_entries` hiện tại rồi dọn orphan/duplicate sau cùng.
...
- Khi ghi lại kết quả sync sau resync, backend **chỉ cập nhật** `googleCalendarEventId`/`meetLink` trên đúng row entry active theo `id`, **không** đụng tới `effectiveFrom`/`effectiveTo` hay các entry đã soft-close (lịch sử đổi lịch giữ nguyên trong bảng, không còn cần backfill `createdAt` như thời JSON cũ).
...
- Makeup event có thể lưu baseline metadata từ lịch cố định: `baselineScheduleEntryId` trỏ tới entry trong `class_schedule_entries`, `originalDate` là ngày của occurrence gốc bị học bù. Backend validate baseline entry còn tồn tại và ngày gốc khớp `dayOfWeek` của slot cố định trước khi tạo/cập nhật metadata.
...
| `[Calendar Resync:Recurring]` | Orchestration log cho resync recurring event từ `class_schedule_entries`; có `state=started/discovered/delete_candidates/existing_event_adopted/create_started/update_started/create_succeeded/update_succeeded/stale_event_detected/sync_failed/quota_limited/delete_started/delete_succeeded/delete_failed/skipped/summary` |
...
| `[Calendar CRUD:sync]`   | Log legacy còn lại khi xóa/tạo recurring event cho `class_schedule_entries`                  |
...
1. **Meet link theo gia sư (luồng mới):**
   ...
   - Xác nhận `entry.meetLink` trong `class_schedule_entries` trả về đúng link gia sư.

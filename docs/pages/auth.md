...
- `GET /users/me/student-wallet-sepay-static-qr` — trả **QR SePay tĩnh** cho học sinh hiện tại; QR không chứa số tiền, nội dung chuyển khoản là `[SEPAY_TRANSFER_NOTE_PREFIX] NAPVI UNIST-{uuid} UNICL-{uuid}... LOP <tên lớp...>` và response có `classIds`. Prefix mặc định rỗng; VietinBank theo hướng dẫn SePay nên dùng `SEVQR`. Frontend hiển thị QR này trực tiếp trong popup nạp ví; webhook mới cộng `account_balance` theo marker `NAPVI` và các id ở đầu nội dung.

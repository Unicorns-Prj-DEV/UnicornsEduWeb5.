...
- `PATCH /users/me/student` — cập nhật hồ sơ học viên (full_name, email, school, liên hệ phụ huynh, …). Body: `UpdateMyStudentProfileDto` (self-service không cho cập nhật `status`). `parent_email` là email nhận biên nhận phụ huynh, không phải email đăng nhập. 400 nếu user không có student.

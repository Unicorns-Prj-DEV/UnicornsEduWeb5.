...
- `/staff/classes/[id]`
  ...
  - `teacher` được phân công lớp có thể tạo buổi bù mới trực tiếp từ `/staff/classes/[id]`, nhưng `gia su phu trach` bị khóa cứng về staff hiện tại. Form có dropdown **Buổi học gốc** chỉ lấy từ card **Cảnh báo chưa dạy** (teacher self-service chỉ thấy cảnh báo của chính mình); chọn buổi sẽ prefill giờ học và lưu baseline metadata (`baselineScheduleEntryId`, `originalDate`). Nếu buổi gốc thuộc cảnh báo chưa dạy, popup **Thêm buổi bù** hiện textarea **Lý do giải trình** bắt buộc và submit một lần lưu giải trình + tạo buổi bù.

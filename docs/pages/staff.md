...
- `/staff/profile`
  ...
  - ô QR (`StaffQrCard`, `size="minimal"`): link Google Drive `/file/d/<ID>/view` (và biến thể `/edit`, `open?id=`, `uc?id=`) được quy về URL ảnh trực tiếp `uc?export=view&id=` trước khi đưa vào `<img>` (helper thuần `apps/web/lib/staff-qr-image.ts`). Bấm ô QR mở `ResponsiveDialog` với mã đủ lớn để quét (ảnh upload phóng to; link không phải ảnh thì xin mã `api.qrserver.com` 512px). Vẫn có nút **Mở link gốc**. Ảnh không tải được hiện thông báo, không để ô trống. Cùng component với admin staff detail.

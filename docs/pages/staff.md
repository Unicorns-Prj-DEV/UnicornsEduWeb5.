...
- `/staff/profile`
  ...
  - trường `Mô tả chuyên môn` (`specialization`) trong popup: textarea nhiều dòng, kéo giãn theo phương thẳng đứng (`resize-y`); hiển thị ở **Thành tích chuyên môn** bằng Markdown trực tiếp từ chuỗi lưu trong DB; raw HTML không được render — đồng bộ admin detail
...
- `/staff/classes/[id]`
  ...
  - card `Danh sách học sinh` tách thành 2 phần: danh sách học sinh **đang học** (có cột **Số dư** theo quyền ở trên) và block riêng **Học sinh đã nghỉ** (không hiện số dư) để tránh lẫn trạng thái
  - `GET /staff-ops/classes/:id` redact `accountBalance` theo access mode: elevated (admin/assistant/kế toán) giữ đủ; `customer_care` chỉ giữ học sinh được gán; `teacher` / `training_manager` bỏ hết.
...
- `/staff/assistant-detail`, `/staff/accountant-detail`, `/staff/communication-detail`, `/staff/technical-detail`, `/staff/training-detail`
  ...
  - riêng `/staff/training-detail` (self + admin mirror `?staffId=`): thêm tab **Lớp học** bên cạnh **Trợ cấp** — bảng lớp được gán quản lý với `MonthInput`, cột tổng tháng + chưa thanh toán all-time, deep-link `/staff/classes/[id]`; dữ liệu từ `GET /training-manager/staff/:staffId/managed-classes?month=`

...
- `/staff/users`, `/staff/staffs`, `/staff/staffs/[id]`, `/staff/classes`, `/staff/students`, `/staff/students/[id]`, `/staff/deductions`, `/staff/costs`, `/staff/history`
  ...
  - khi `id` trùng `staffInfo.id` của user hiện tại (trang **Cá nhân**): **không** hiển thị chỉnh sửa kiểu admin (nút **Chỉnh sửa thông tin nhân sự**, popup `EditStaffPopup`, chỉnh QR thanh toán trong `Hồ sơ nhân sự`); tự cập nhật hồ sơ qua `/staff/profile` hoặc `/user-profile`

...
- `/staff/users`, `/staff/staffs`, `/staff/staffs/[id]`, `/staff/classes`, `/staff/students`, `/staff/students/[id]`, `/staff/deductions`, `/staff/costs`, `/staff/history`
  ...
  - riêng `/staff/staffs/[id]` mirror đầy đủ staff detail của admin, gồm cả nút **Thanh toán** ở header cho `assistant` / `accountant`; popup **Thanh toán** dùng cùng contract backend `GET /staff/:id/payment-preview` và `PATCH /staff/:id/payment-status/pay-all`, giữ nguyên rule: **buổi dạy (GV)** trong popup/pay-all là **mọi buổi chưa thanh toán** (không giới hạn tháng đang xem), các nguồn khác vẫn theo tháng query; **bonus** là section riêng, session `deposit` không nằm trong đợt thanh toán, và thuế trong popup luôn tính theo mức hiện hành của staff theo từng role tại thời điểm thanh toán
...
- `/staff/profile`
  ...
  - trường `Mô tả chuyên môn` (`specialization`) trong popup: textarea; hiển thị ở **Thành tích chuyên môn** dùng Markdown (mặc định) hoặc HTML đã sanitize khi chuỗi có thẻ HTML — đồng bộ admin detail
  ...
  - hiển thị đầy đủ các section cùng contract dữ liệu với admin detail:
    - Hàng đầu dùng `StaffIdentityOverview`: card đồng bộ style các section khác, QR minimal cùng hàng tiêu đề, khối **Thành tích chuyên môn** (`specialization`) nền phụ: component `StaffSpecializationRichText` — HTML sanitize hoặc Markdown; trước khi parse Markdown, chuỗi một dòng kiểu `… . - ` / `…:- ` / `**Tiêu đề:** - ` được chèn xuống dòng trước `- ` để thành list; đồng bộ với admin staff detail
...
- **Backend routes đang dùng**
  ...
  - `PATCH /staff/:id/payment-status/pay-all` — assistant/accountant trên route mirror có thể thanh toán đúng tập liệt kê trong preview (mọi buổi GV chưa thanh toán + pending tháng đó cho các nguồn khác); backend tự tính lại trong transaction, refresh snapshot thuế hiện hành theo role, rồi chuyển sang `paid`; riêng nguồn giáo viên dùng công thức `gross - operating` rồi mới áp thuế; FE không gửi danh sách id từ client

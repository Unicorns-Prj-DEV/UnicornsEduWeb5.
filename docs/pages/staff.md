...
- **Backend routes đang dùng**
  ...
  - `GET /fixed-salary-settings/staff-overrides?staffId=`, `PUT /fixed-salary-settings/staff-overrides/amount`, `PUT /fixed-salary-settings/staff-overrides/operating-rate` — đè theo cặp (nhân sự, role); PUT từng trục giữ cho trang cấu hình lương chung. UI mức đè nằm trong dialog **Chỉnh sửa thông tin nhân sự** (`/staff/staffs/[id]`, mirror admin) và ghi qua `PATCH /staff/:id/with-fixed-salary-overrides` (vai trò + mức đè một transaction). Hai trục độc lập; trống = mặc định vai trò; `0` = cố ý loại. Tắt vai trò đang có mức đè thì `ConfirmDialog` nêu đúng số sắp mất; hủy thì vai trò vẫn bật; tháng đã chốt không đổi.

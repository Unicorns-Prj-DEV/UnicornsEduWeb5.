...
- **Backend routes đang dùng**
  ...
  - `GET/PUT /fixed-salary-settings/role-defaults` và `GET/PUT /fixed-salary-settings/role-operating-rates` — tab **Lương cứng** (một bảng 3 cột, hai nút lưu độc lập theo hai API); **không gồm giáo viên**; admin + assistant xem/sửa; không đụng trợ cấp buổi học
  ...
  - `GET /fixed-salary-settings/payables`, `POST /fixed-salary-settings/close-month` — chốt lương cứng tháng hiện tại + xem khoản đã đóng băng; **không sinh khoản cho giáo viên**; admin + assistant; cron ngày 28 cùng hàm

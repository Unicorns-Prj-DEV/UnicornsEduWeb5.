...
- `class_surveys`: báo cáo khảo sát của một lớp cho một Bài khảo sát (`survey_id`, nullable — chỉ null cho data lịch sử trước khi có Bài khảo sát). Field legacy `test_number`/`content` (lần khảo sát N toàn cục + nội dung rich text tự do) giữ nullable cho dữ liệu cũ, không còn set qua API mới. Index `class_id`, `teacher_id`, `(class_id, test_number)`, `(teacher_id, report_date)`, `survey_id`, `(class_id, survey_id)`. Unique effective theo `(class_id, survey_id)` được enforce ở service layer (một báo cáo/lớp/bài khảo sát)
- `class_survey_student_assessments`: đánh giá kiến thức (`knowledge_assessment`, text tự do) + nhận xét (`comment`, text tự do) của gia sư cho từng học sinh trong một `class_surveys` row; unique `(class_survey_id, student_id)`
